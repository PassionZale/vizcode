...
const LOCATIONS = [
  {
    city: "武汉",
    rectPosition: [-150, -340] as RectProps["position"],
    flagWebm: targetWebm,
    flagPosition: [-150, -340] as RectProps["position"],
  },
  {
    city: "深圳",
    rectPosition: [-20, 330] as RectProps["position"],
    flagWebm: targetWebm,
    flagPosition: [-20, 330] as RectProps["position"],
  },
];
  ...
  locationRefs.map((ref, i) => {
    contentRef().add(
      <Rect ref={ref} position={LOCATIONS[i].rectPosition} opacity={0}>
        <Txt fill={"red"} fontSize={32} fontWeight={700}>
          {LOCATIONS[i].city}
        </Txt>
      </Rect>
    );
